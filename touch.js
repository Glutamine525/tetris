    ...
    if (
        ...
        if (isGameRunning()) {
            ...
            for (var i = 0, l = e.touches.length; i < l; i++) {
                var tX = e.touches[i].pageX,
                    tY = e.touches[i].pageY;
                for (var j = 0; j < touchButtons.length; j++) {
                    var btn = touchButtons[j];
                    if (
                        tX >= btn.offsetLeft &&
                        tX < btn.offsetLeft + btn.offsetWidth &&
                        tY >= btn.offsetTop &&
                        tY < btn.offsetTop + btn.offsetHeight
                    ) {
