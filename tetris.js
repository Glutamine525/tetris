...
var tdFinesseElement = $$("td-finesse");
var finesseElement = $$("finesse");
...
function resize() {
    ...
    if (
        ...
        if (screenWidth > winWidth) screenHeight = ~~(window.innerWidth / 1.0);
        ...
        stackCanvas.width = activeCanvas.width = bgStackCanvas.width =
            cellSize * 10;
...
function init(gt, params) {
    if (gt === "replay") {
        ...
        if (params !== void 0) {
            try {
                ...
                if (typeof replay !== "object") throw "json parse fail";
                if (
                    replay.gametype === void 0 ||
                    replay.keys === void 0 ||
                ...
                try {
                    ...
                    alert(
                        "replay too old, failed to auto-upgrade... 回访数据过于古代，自动升级失败...\n" +
                            e
                    );
                    return;
                }
                replay.keys = keysDecode(replay.keys);
                if (replay.keys === null) throw "keys decode fail";
            } catch (e) {
                alert(
                    "invalid replay data... 回放数据有误...\n" + e.toString()
                );
                return;
            }
        }
    ...
    finesseElement.innerHTML = statsFinesse = 0;
    //TODO Check if needed.
    piece = new Piece();
    // if (settings.ForceFinesse) {
    //     tdFinesseElement.style.display = "block";
    // } else {
    //     tdFinesseElement.style.display = "none";
    // }
    ...
    if (gametype === 4) {
        ...
        if (
            ...
            if (!s) {
                s = ".aaaa.aa.. .a.a.a..a. .a.a.a.aa. .a.a.aa.a. .a.a.a.aa. a...a..... aa.a.a...a a.a.a.a.a. a.a.a..a.. a.a.a.a.a. .aa.aa...a .......... ..4.4.4... ..b.b.b... ..b.b.b... .9999999.. .4434443.. .3444344.. "
                    .replace(/a/g, "" + (1 + ~~(rng.next() * 7)))
                    .replace(/b/g, "" + (1 + ~~(rng.next() * 7)));
            }
...
function statisticsStack() {
    ...
    statsScore.style.textShadow =
        combo === 0 ? "" : "0 0 0.5em " + light[(combo - 1) % 10];
...
function update() {
    ...
    do {
        ...
        if (gametype === 3) {
            ...
            var fromLastHD =
                flags.hardDrop & keysDown ? frame - frameLastHarddropDown : 0;
            var curStage = 0,
                objCurStage;
            while (
...
function gameLoop() {
    ...
    if (isGameRunning()) {
        ...
        var repeat =
            ((Date.now() - startTime - pauseTime) / 1000) * fps - frame;
        if (repeat >= 2) {
        ...
        statistics(); // as you draw for lock delay brightness gradient... give this up..
...
function checkWin() {
    ...
    if (gametype === 1) {
        // Marathon
        if (settings.Gravity !== 0 && lines >= 200) {
        ...
        // misc line limited modes
        if (lines >= lineLimit) {
            isend = true;
        }
    }
    if (gameparams.symmetry === 1) {
        // you must end symmetrically
        var halfwidth = Math.ceil(stack.width / 2);
        outer: for (var x = 0; x < halfwidth; x++) {
            var symmx = stack.width - 1 - x;
            for (var y = 0; y < stack.height; y++) {
                var c = stack.grid[x][y],
                    symmc = stack.grid[symmx][y];
...
function trysubmitscore() {
    ...
    if (gametype === 4 && gameparams.digraceType === "map") void 0; //return;
    var obj = { req: "ranking" };
    var time = scoreTime;
    if (timePenalty) {
    ...
    if (gametype === 0)
        ...
        obj.mode =
            ...
            (gameparams.pieceSet
                ? ["", "noi", "alli"][gameparams.pieceSet]
                : "") +
            (gameparams.backFire
                ? ["", "bf1", "bf2", "bf3", "bf4"][gameparams.backFire]
                : "") +
        ...
        obj.mode =
            "marathon" +
            (gameparams.marathonType
                ? ["", "cls"][gameparams.marathonType]
                : "");
    else if (gametype === 5)
        // score attack
        obj.mode = "score";
