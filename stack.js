...
Stack.prototype.addPiece = function (piece) {
    ...
    // for (var tx = 0; tx < piece.tetro.length; tx++) {
    //     for (var ty = 0; ty < piece.tetro[tx].length; ty++) {
    //         var x = tx + piece.x,
    //             y = ty + piece.y;
    //         if (piece.tetro[tx][ty] && y >= 0) {
    //             if (!once || x < column) {
    //                 column = x;
    //                 once = true;
    //             }
    ...
    // console.log(
    //     piece.finesse,
    //     finesse[piece.index][piece.pos][column],
    //     piece.finesse - finesse[piece.index][piece.pos][column]
    // );
    // if (piece.finesse - finesse[piece.index][piece.pos][column] !== 0) {
    //     return;
    // }
    ...
    do {
            ...
            for (var ty = 0; ty < tetro[tx].length; ty++) {
                ...
                if (tetro[tx][ty] && y >= 0) {
                    ...
                    if (gameparams.symmetry === 1) {
                        ...
                        if (c !== void 0 && symmc !== void 0) {
                            // in-game, pass if only one
        ...
        if (lineClear !== 0) {
            ...
            if (isSpin) {
                scoreAdd = scoreAdd.mul(
                    bigInt(400 + 400 * lineClear).mul(
                        bigInt(2).pow(b2b + combo)
                    )
                );
                garbage = lineClear * (b2b == 0 ? 2 : 3);
                b2b += 1;
                sound.playse("tspin",combo, Math.min(lineClear, 3));
            } else if (lineClear >= 4) {
                scoreAdd = scoreAdd.mul(
                    bigInt(
                        100 * ((lineClear * lineClear) / 4 + lineClear) -
                            25 * (lineClear & 1)
                    ).mul(bigInt(2).pow(b2b + combo))
                );
                garbage = ~~((lineClear * 1.5 - 2) * (b2b == 0 ? 1 : 1.25));
                b2b += 1;
                sound.playse("erase4",combo);
            } else {
                scoreAdd = scoreAdd.mul(
                    bigInt([100, 300, 500, 800][lineClear - 1]).mul(
                        bigInt(2).pow(combo)
                    )
                );
                b2b = 0;
                garbage = [0, 1, 2, 4][lineClear - 1];
                sound.playse("erase",combo, lineClear);
            }
            ...
            if (isSpin) {
                scoreAdd = scoreAdd.mul(
                    bigInt(2).pow(bigInt(b2b)).mul(bigInt(400))
                );
                sound.playse("tspin0");
            } else {
                scoreAdd = bigInt(0);
        ...
        var isPC = true;
        if (piecesDel !== 0) {
            // self symmetry violation leads to fake PC
            isPC = false;
        }
        for (var x = 0; x < this.width; x++)
            for (var y = 0; y < this.height; y++)
                if (this.grid[x][y]) isPC = false;
        if (isPC) {
        ...
        if (gameparams.backFire) {
            ...
            if (garbage !== 0) {
                if (gameparams.backFire === 1) {
                    ...
                    gameparams.backFire === 2 ||
                    gameparams.backFire === 3
                ) {
                    ...
                    if (isPC) {
                        ...
                        $setText(
                            msg,
                            ["DOUBLE!", "TRIPLE!", "TETR.JS!"][lineClear - 2]
                        );
                    }
                }
            }
    ...
    // console.log(piece.finesse,finesse[piece.index][piece.pos][column],piece.finesse - finesse[piece.index][piece.pos][column])
...
Stack.prototype.delWholePieceAt = function (x, y) {
    ...
    while (qh < queue.length) {
        ...
        if (c !== void 0) {
            // may be added into queue twice or more
            this.grid[cx][cy] = void 0;
            if (
                c & cellFlags.connRight &&
                cx + 1 < this.width &&
                this.grid[cx + 1][cy] !== void 0
            ) {
                queue.push([cx + 1, cy]);
            }
            if (
                c & cellFlags.connDown &&
                cy + 1 < this.height &&
                this.grid[cx][cy + 1] !== void 0
            ) {
                queue.push([cx, cy + 1]);
            }
            if (
                c & cellFlags.connLeft &&
                cx > 0 &&
                this.grid[cx - 1][cy] !== void 0
            ) {
                queue.push([cx - 1, cy]);
...
Stack.prototype.draw = function () {
    ...
    if (this.statisticsDirty) statisticsStack();

    this.dirty = false;
};
Stack.prototype.makeSomeArt = function (piece) {
    var arrRow = [8, 8, 8, 8, 8, 8, 8, 8, 8, 8];
    for (var y = 0; y < this.hiddenHeight + 1; y++) {
        // ensure dies
        this.rowRise(arrRow, piece);
    }
    var colors = [];
    var dc = ~~(rng.next() * rng.next() * 6) + 1,
        bc = ~~(rng.next() * 7);
    for (var c = 0; c < 7; c++) {
...
Stack.isMinoSymmetric = function (c, symmc) {
    ...
    // exchange left&right
    conn =
        (conn & cellFlags.connUp) |
        (conn & cellFlags.connDown) |
