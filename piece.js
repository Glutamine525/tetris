    ...
    if (gametype === 6) {
        ...
        if (gameparams.marathonType === 1) {
            ...
            if (level < 20) {
                ...
                this.gravity = Infinity;
                this.lockDelayLimit = ~~(
                    30 * Math.pow(0.93, Math.pow(level - 20, 0.8))
                ); // magic!
            }
        }
...
Piece.prototype.tryKickList = function (
    kickList,
    rotated,
    newPos,
    offsetX,
...
Piece.prototype.rotate = function (direction) {
    ...
    var offsetX = offset[newPos][0] - offset[curPos][0];
    var offsetY = offset[newPos][1] - offset[curPos][1];
    if (settings.RotSys === 2 || settings.RotSys === 14) {
        //ARS, Plus
        var kickList = [];
        if (this.index === PieceI.index) {
            ...
            if (
                newPos === 0 ||
                ((this.index === PieceS.index || this.index === PieceZ.index) &&
                    newPos === 2)
            )
                kickList = [
                    [0, 0],
        ...
        else if (settings.RotSys === 1) kickList = WKTableCultris;
        else if (settings.RotSys === 15)
            kickList = WKTableDX[kickIndex][curPos];
        else kickList = WKTableDTET[kickIndex];
        this.tryKickList(kickList, rotated, newPos, offsetX, offsetY);
    }
};
...
Piece.prototype.checkFall = function () {
    ...
    /* farter */ if (Math.abs(this.y - Math.round(this.y)) < 0.000001)
...
Piece.prototype.checkLock = function () {
    if (landed) {
        //check if it is finesse
        var once = false;
        for (var tx = 0; tx < this.tetro.length; tx++) {
            for (var ty = 0; ty < this.tetro[tx].length; ty++) {
                var x = tx + this.x,
                    y = ty + this.y;
                if (this.tetro[tx][ty] && y >= 0) {
                    if (!once || x < column) {
                        column = x;
                        once = true;
                    }
        ...
        // console.log(
        //     this.finesse,
        //     finesse[this.index][this.pos][column],
        //     this.finesse - finesse[this.index][this.pos][column]
        // );
        if (
            this.finesse - finesse[this.index][this.pos][column] > 0 &&
            this.lockType > 0
        ) {
            statsFinesse++;
            finesseElement.innerHTML = statsFinesse;
            if(settings.ForceFinesse || settings.FinesseSound){
                sound.playse("fault");
            }
            if (settings.ForceFinesse) {
                tmp = this.held;
                piece.new(this.index);
                piece.held = tmp;
                return;
            }
        }
    ...
    if (!this.dead) {
        ...
        if (settings.Ghost !== 3) {
            ...
            draw(
                ...
                activeCtx,
                RotSys[settings.RotSys].color[this.index],
                a
            );
        }
    }
    ...
    if (!landed) {
        ...
        if (settings.Ghost === 0 || settings.Ghost === 1) {
            ...
            draw(
                this.tetro,
                this.x,
                Math.floor(this.y + this.getDrop(Infinity)) -
                    stack.hiddenHeight,
                activeCtx,
                color
            );
